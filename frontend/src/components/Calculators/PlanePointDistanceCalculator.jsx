...
export default function PlanePointDistanceCalculator() {
  ...
  const handleSubmit = async () => {
    ...
    const hasPoint = [point.x, point.y, point.z].some(
      (v) => parseFloat(v) !== 0
    );
    ...
    try {
      const response = await calculateDistanceFromPointToPlane(transformedPlane, point);
      setResult(response);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="geometry-wrapper">
            ...
            <>
              <div className="geometry-result">
                ...
                <h3 style={{ marginTop: "20px" }}>
                  Udaljenost točke od ravnine:
                  <br />
                  <span>{result.result?.toFixed(4)}</span>
                </h3>
              </div>
        ...
        >
          <PlanePointDistanceVisualizer plane={plane} point={point} />
        </FullscreenModal>
      </div>
    </>
  );
