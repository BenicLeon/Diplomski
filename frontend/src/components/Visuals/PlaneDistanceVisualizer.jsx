...
const sketch = (p5) => {
  ...
  p5.setup = () => {
    ...
    zoom = isMobile ? 0.5 : 1;
    angleX = isMobile ? -1.2 : 0;
    angleY = isMobile ? 1.1 : 0;
  };
  ...
  p5.draw = () => {
    ...
    if (plane1) drawPlane(p5, plane1, [255, 0, 0, 100], "π₁");
    if (plane2) drawPlane(p5, plane2, [0, 0, 255, 100], "π₂");
  };
  ...
  function drawPlane(p5, plane, color, labelText) {
    ...
    const u = normal.copy().cross(p5.createVector(0, 0, 1)).normalize().mult(size);
    ...
    p5.fill(...color);
    p5.beginShape();
    p5.vertex(-u.x, -u.y, -u.z);
    p5.vertex(u.x, -u.y, -u.z);
    p5.vertex(u.x, u.y, u.z);
    p5.vertex(-u.x, u.y, u.z);
    p5.endShape(p5.CLOSE);
    label(p5, labelText, 0, 0, 0, isMobile ? 10 : 16);
    p5.pop();
  }
...
const PlaneDistanceVisualizer = ({ plane1, plane2 }) => {
...
export default PlaneDistanceVisualizer;
